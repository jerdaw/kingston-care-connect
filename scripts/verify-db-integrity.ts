...
async function verifyIntegrity() {
    ...
    const { data: services, error: fetchError } = await supabase
        .from('services')
        .select('id, name, embedding') as { data: { id: string, name: string, embedding: string | null }[] | null, error: any };
    ...
    console.log(`🧠 Embedding Coverage: ${(coverage * 100).toFixed(1)}% (${servicesWithEmbeddings.length}/${services.length})`);
